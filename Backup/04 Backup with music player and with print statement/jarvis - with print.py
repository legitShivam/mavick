...
def createMusicControler(songName):
    if isSongPlaying:
        ...
        stopStatus = eel.stopStatus()()
        while stopStatus == 'False':
            playStatus = eel.playStatus()()
            stopStatus = eel.stopStatus()()
            if playStatus == 'False':
                AudioPlayer.pause()
            elif playStatus == 'True':
                AudioPlayer.resume()
            eel.sleep(0.3)  # Lowers the CPU load
        AudioPlayer.stop()
        ...
        if sleep == False:
            ...
            if "tell me the time" in query or "what is the time" in query:  # Giving currrent time
                ...
                if f"search {wikipediaSearchKey} on wikipedia" in query:  # Searches the wikipedia
                    ...
                    print("   --- Result: \n", results, "\n")
                    speak(results)
                    eelStatus(f"Do you want detail information about it")
                            ...
                            if f"search {search} on youtube" in query or "on youtube" in query:
                                ...
                                if f"search {search} on google" in query or "on google" in query or f"what is {search}" in query:
                                    try:
                                        ...
                                        webbrowser.open(
                                            f"https://www.google.com/search?q=hello+world&oq=hello+world{search}")
                                    except:
                                        pass
                                    ...
                                    choosedMusicName = query.replace('play music', '').replace(
                                        'play', '').replace('play song', '')
                                    if f'play music {choosedMusicName}' in query or f'play song {choosedMusicName}' in query or f'play {choosedMusicName}' in query:
                                    ...
                                    elif f'play music' in query.replace(' a', '') or f'play song' in query.replace(' a', '') or f'play random music' in query.replace(' a', '') or f'play random song' in query.replace(' a', ''):
                                        createMusicControler(
